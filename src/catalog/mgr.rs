...
impl CatalogManager {
    ...
    fn check_tags(&self, metadata: &Metadata) {
        ...
        if metadata.gps_altitude.is_none() {
            log::warn!(
                "{}: GPS altitude not assigned.",
                metadata.source_file.display()
            );
        }
        if metadata.gps_altitude_ref.is_none() {
            log::warn!(
        ...
        if metadata.gps_latitude.is_none() {
            log::warn!(
                "{}: GPS latitude not assigned.",
                metadata.source_file.display()
            );
        }
        if metadata.gps_latitude_ref.is_none() {
            log::warn!(
        ...
        if metadata.gps_longitude_ref.is_none() {
            log::warn!(
        ...
        if metadata.make.is_none() {
            log::warn!("{}: Make not assigned.", metadata.source_file.display());
        }
        ...
        // If not, Artist & Copyright aren't important.
            ...
            if metadata.model.is_none() {
                log::warn!("{}: Model not assigned.", metadata.source_file.display());
            }
            if metadata.artist.is_none() {
                log::debug!("{}: Artist not assigned.", metadata.source_file.display());
            }
            if metadata.copyright.is_none() {
                log::debug!(
    ...
    fn is_my_camera(&self, metadata: &Metadata) -> bool {
        self.my_cameras
            .contains(&metadata.model.as_deref().unwrap_or_default())
    }
    ...
    fn new(directory: &Path, trash: Option<&Path>) -> Self {
        ...
        // Helpers.
        let image_exts = HashSet::from(Self::LIVE_PHOTO_IMAGE_TYPES);
        let is_image =
            |metadata: &Metadata| -> bool { image_exts.contains(&metadata.file_type.as_str()) };
        let video_exts = HashSet::from(Self::LIVE_PHOTO_VIDEO_TYPES);
        let is_video =
            |metadata: &Metadata| -> bool { video_exts.contains(&metadata.file_type.as_str()) };
        ...
        for media in media_files.values() {
            if let Some(id) = &media.metadata.content_identifier {
                ...
                } else if is_video(&media.metadata) {
                    log::debug!(
        ...
        Self {
            ...
            live_photo_images,
            live_photo_videos,
            my_cameras: HashSet::from(Self::MY_CAMERAS),
        }
    }
    ...
    pub fn check_media_file_tag_validity(&self) {
        log::info!("Checking that all files have required tags.");

        for media in self.media_files.values() {
            self.check_tags(&media.metadata);

            if let Some(xmp) = &media.xmp {
                self.check_tags(&self.xmps.get(xmp).unwrap().metadata);
            }
        }
    }
    ...
    pub fn finalize_move_and_rename_files(mut self, destination: &Path) {
        ...
        for (media_path, media) in &self.media_files {
            ...
            let datetime_tag = if metadata.date_time_original.is_some() {
                ...
                log::error!(
                    "{}: Cannot rename without DateTimeOriginal set. Skipping.",
                    media_path.display()
                );
                continue;
            };
